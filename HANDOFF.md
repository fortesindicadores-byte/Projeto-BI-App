...
- **Fatos** (indicadores com desvio) vêm de: ICs do Gerot + Disponibilidade (planilha ICs, abaixo da meta 90%); da planilha **Km/L por projeto** (col14): **Consumo Km/L** (real<rem, nominal Km/L) e **R$/L** (novo indicador — só quando preço **real > rem**, nominal R$/L); e **desvios financeiros** da DRE. **IMPORTANTE: custos na DRE são NEGATIVOS**; desvio (gasto acima do orçado) = `remunerado − realizado > 0`; magnitude = esse valor, % = mag/|rem|. Excluídos sempre: Receita Líquida, ICMS Crédito Presumido, IPVA e Licenciamento, Estorno ICMS.
...
- **Auto-causa (kickstart do preenchimento):** Consumo Km/L já pré-preenche a Causa com as **3 piores placas** (Rem Modelo | Rem Médio | Real), por projeto — só quando a ação está vazia (`f.causaAuto` + `loadAcoes(id,causaAuto)`). **A FAZER:** Combustível (DRE) pré-preencher a Causa com os **maiores impactos em ordem** (drivers da árvore de combustível): Dispersão de km (do sheet "Dispersão de km": col33 Δkm, col34 %, col36 Impacto BRL — pronto), + Consumo Km/L e Preço R$/L (decomposição em R$ — replicar a lógica do painel **Árvore de Combustível** p/ os números baterem; NÃO inventar fórmula). Ex.: "1. Dispersão de km: ▲700km | ▲8% | Impacto 37k".
