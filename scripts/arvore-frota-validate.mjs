...
const suf=v=>{const x=String(v||'');const s=x.includes('-')?x.split('-').slice(1).join('-').trim():x.trim();return s.replace(/\s*\(INATIVO\)\s*/i,'').trim();};
...
async function main(){
  ...
  const DP={vig:0,proj:fb(fi(dp.cols,'proj.','projeto'),14),kmRem:fb(fi(dp.cols,'km rem'),31),kmReal:fb(fi(dp.cols,'km rodado tt','km rodado'),32),ativa:fb(fi(dp.cols,'frota ativa','ativa'),21)};
  console.log('\n== COLUNAS DETECTADAS ==');
