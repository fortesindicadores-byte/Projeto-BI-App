...
- **Custos por PACOTE (fca-preenchimento):** o fato gerado é o **pacote líquido** que estourou vs remunerado (`Pacote Manutenções` + `Desvio: ▲ R$ X · ▲ -Y%`); a **causa automática é SÓ as contas** do pacote (rótulo `Contas:`), do maior desvio ao maior saving — ▲ estouro (vermelho) · ▼ saving (verde), coloridos nas visões de leitura via helper `tri()`. **A partir de 08/2026 vale para TODOS os pacotes** (Renan, 14/08): saíram os drivers de Combustíveis (Dispersão de km, Km/L com bottom 3 placas, R$/L) e o custo/placa ativa de Manutenções/Pneus — viravam miscelânea no card; o driver fica no painel do indicador. Por isso a geração não baixa mais `Dispersão de km` nem `Km/L`. **Estorno de ICMS não Aproveitado e ICMS Crédito Presumido NÃO são do pacote Combustíveis** — vão para `ICMS`, como na Visão Financeira (o `EXCL` do pushDRE tinha a chave sem o "DE" e nunca casava, por isso o Estorno aparecia na causa). Vale para as **próximas gerações**; o histórico por conta fica como está (não re-rodar `?gen=all` em vigência já gerada no modelo antigo — criaria os dois formatos).
...
**A ENXUGADA FOI DESFEITA (Renan, 14/08/2026):** a versão larga das abas foi restaurada — `KmPorLitro` e `PrecoDiesel` voltaram à Base Remunerado e os painéis de combustível da Seara voltaram a usá-los como benchmark (a variante "tudo por J+O" de 13/08 foi revertida). **Fluxo do fechamento do mês:** o Renan cola o mês novo primeiro na aba **Combustível**; Base Remunerado e Base CTEs chegam depois. Enquanto o mês não existe nelas, os painéis usam o **mês anterior da mesma placa** para as TAXAS (KmPorLitro, PrecoDiesel e o peso de viagens) — quantidade (km remunerado da J) NÃO cai no mês anterior, fica vazia até o mês ser colado. As colunas da Base Remunerado são achadas pelo **nome do cabeçalho** (fallback nos índices largos 15/17), e a data da Base CTEs é detectada entre **D** (layout enxuto) e **E** (layout largo) pelo que parseia como data — os painéis aguentam os dois layouts.
