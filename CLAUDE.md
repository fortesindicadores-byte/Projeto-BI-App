...
**AS ABAS FORAM ENXUGADAS (confirmado por xlsx em 13/08/2026):** Base Remunerado termina na col **O** (`ReaisPorKm`) — `KmPorLitro`, `PrecoDiesel`, `ReaisPorKmDiesel` etc. NÃO existem mais; Base CTEs termina na col **J**. **TODO o remunerado da Seara sai de J + O** (regra do Renan): Km/L Rem = **km rem (J dedup) ÷ litros abastecidos**; Impacto do Km/L = **(km real − km rem) × ReaisPorKm(O)** — aditivo, a soma das placas/unidades é o total. Na Árvore Seara, R$/L rem = |custo rem DRE| ÷ litros (a árvore fecha: R$/L ÷ Km/L = R$/km). As colunas da Base Remunerado são achadas pelo **nome do cabeçalho**, nunca por índice.
