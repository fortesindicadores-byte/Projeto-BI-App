...
- **LISTA (padrão)** — clusters empilhados, cada um é um painel de vidro com cabeçalho clicável e um **"+" que vira "−"** ao abrir. Dentro, **só o nome do painel** (sem card): linhas de texto em `flex-wrap`, com uma bolinha que acende em laranja no hover e a descrição no `title`. Assim os **7 clusters cabem abertos ao mesmo tempo** — por isso a primeira visita já abre tudo (`gem_hub_abertos` ausente ⇒ todos). Validado em 1600×900 e 1366×768. Duas armadilhas de flex: `.board.lista .clu{flex:0 0 auto}` (senão os painéis encolhem e cortam em vez de rolar) e `.board.lista .clu-cards{flex-direction:row}` (senão herda o `column` do modo colunas e empilha um nome por linha).
